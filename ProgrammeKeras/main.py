...
model.add(Dense(100, activation='sigmoid', input_dim=3,name='couche1'))
#apres la premiere couche il n'est pas necessaire de specifier imput_dim
model.add(Dense(800, activation='sigmoid',name='couche2'))
model.add(Dense(100, activation='linear',name='couche3'))
model.add(Dense(600, activation='sigmoid',name='couche4'))
model.add(Dense(1, activation='linear', name='couche5'))
...
history = model.fit(x_train, y_train,epochs=200,batch_size=6, verbose=1,shuffle=True ) #, callbacks=[early_stopping])
...
#PREDICTION
previsions = model.predict(np.array([[400,401,402]]))
